...
`build_assets_v2.py` builds the text-cleaned runtime archive from the exact published `assets-v1` ZIP. It requires Python 3.11 or newer and the Python standard library.

```powershell
gh release download assets-v1 --pattern darkeden-assets-v1.zip --dir build/assets-v2
python tools/assets/build_assets_v2.py build/assets-v2/darkeden-assets-v1.zip build/assets-v2/darkeden-assets-v2.zip
```
...
Validation reparses the rewritten tables, checks idempotence, reads every ZIP entry to verify its CRC, and compares every unmodified file with v1 by SHA-256. The v1 archive layout is retained, including its empty `UserSet/`; no local saves or account settings enter the archive. Raster artwork is unchanged.
