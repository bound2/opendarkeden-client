...
| R14 | Live `GetTickCount()`/`timeGetTime()` calls under `Client` and `VS_UI` | **186** | The clocks work of the C++20 assessment (priority 5): the 32-bit tick wraps every 49.7 days, and every `previous + delay <= now` over it fires early or stalls across the wrap. Counted by `tests/tools/count_tick_reads.pl`, a character-level scanner over comments and string literals, because the regex strip the clocks slices first counted with reads `//*pDest` as a block opener and swallowed live code (it missed the `srand(GetTickCount())` in `VS_UI_Item.cpp` outright, and read 209 where this reads 215). `basic/` is outside the count: its hits are the definitions and `MonotonicClock`'s own reader. 215 before the widget timers outside `GameCommon` moved to `MonotonicClock::IntervalTimer`; 100 of the 186 are the two `VS_UI_GameCommon` sources. Added 2026-09-10. |
