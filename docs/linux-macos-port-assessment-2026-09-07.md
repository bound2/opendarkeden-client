...
| G. macOS | 2-4 days | Built without a Mac, on GitHub's arm64 runner (`.github/workflows/macos.yml`, `macos` and `macos-asan` presets). Small, as estimated: one include spelling (`<SDL.h>`, the only one every SDL2 package's CMake target guarantees - Homebrew's autotools config exports no parent directory, so `<SDL2/SDL.h>` did not resolve on Apple Silicon), the two global `-I/opt/homebrew` hacks gone, `SDL2main` unlinked off Windows (it renames nothing there), an optional `.app` bundle with the data-root search that Finder and a bundle need (`Basic::FindDataRoot`), `SIGPIPE` ignored at the entry point (macOS has no `MSG_NOSIGNAL`), the system font list, and the Retina case the assessment flagged: the window now asks for `SDL_WINDOW_ALLOW_HIGHDPI` and the mouse mapping goes through the point-to-pixel ratio, pinned by `tests/unit/test_present_geometry.cpp` since no test machine has such a display. A Clang + libc++ build in the Linux container was the local proxy for Apple Clang and found nothing; the source audit for glibc-only calls found nothing either. Nothing has been watched on a Mac's display. |
