...
>
> **Remaining:** the non-const `operator[]` and `Get` still return a mutable reference to a process-wide shared static. Writes through an invalid index can poison later invalid reads; the accessor contract and its callers still require correction.
