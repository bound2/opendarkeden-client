...
| Length-prefixed string writes cap the length after narrowing it to a `BYTE` | the shape `CGSkillToNamed` was fixed for on 2026-09-06 recurs in the other `BYTE`-prefixed string writers: `CGSay::write` does `BYTE szMessage = m_Message.size();`, tests that against 128, then writes `m_Message` whole, so a 300-character chat line narrows to 44, passes the cap, and goes out as 300 bytes behind a length byte claiming 44. Reachable from the chat box if the input is not bounded below 256 upstream of the packet. `CGWhisper`, `CGGlobalChat` and the guild packets share the shape and are unaudited. Found by the adversarial review of the header-rollback fix on 2026-09-06 | open |
