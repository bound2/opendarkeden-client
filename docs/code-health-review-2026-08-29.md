...
> ⚠️ **Repeated work fixed; encoding policy remains open (2026-09-21):** a bounded normalization cache shares results across real measurement, wrapping and drawing calls, including rejected inputs. It owns complete input keys (embedded NULs included), retains at most 1,024 entries and 1 MiB of string capacity per thread, and bypasses oversized inputs. A separate 4,096-entry glyph-metric cache shares successful and failed measurements across colors; a bitmap rendered first supplies fallback dimensions without another white-surface render. LRU tests cover both budgets, key ownership, font separation, eviction and disabled caches. Counters reproduce the repeated work before the fix and prove reuse afterward. The remaining declared-encoding/load-time-conversion recommendation belongs to the still-open encoding finding above; this change preserves the existing decoding policy.
