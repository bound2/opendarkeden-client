...
> **In progress (2026-09-21):** `project_warnings` now explicitly supplies `/W3 /w14311 /w14312 /w14668` or `-Wall -Wextra -Wundef` to every compiled target. The generated C/C++ option inventory is checked by `warning_policy`; `warning_budget_parser` guards the diagnostic counter. Both verification scripts use clean builds, publish counts, and reject growth or unrecorded decreases. Platform baselines are still being measured before this finding can close. The original assertion that MSVC warnings were off was incorrect: CMake already supplied `/W3`. MSVC ASan is also active in the current dedicated build tree.
