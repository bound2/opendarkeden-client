...
| `C_VS_UI_QUEST_STATUS::ShowDesc` and `C_VS_UI_LOTTERY_CARD::ShowDesc` copy up to 2 KB of description into `char[512]` | each loop iteration copied 2047 bytes of the text, or all of the remainder, into a 512-byte stack buffer before cutting one line out of it, so any quest or lottery description over 511 bytes smashed the stack. Latent today only because `Data/ui/txt` is still packed (the CLAUDE.md trap) and the descriptions are absent at runtime. Found by Apple Clang's `-Warray-bounds` in the macOS port, 2026-09-08 - MSVC, GCC 13 and Clang 18 never flagged it | fixed on `port/macos` |
