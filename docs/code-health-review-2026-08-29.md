...
| `Execute_UI_CHAT_RETURN` copies the chat line into a 128-byte heap block with `strcpy` | `Client/UIMessageManager.cpp` ~2555 does `char* strOrg = new char[128]; strcpy(strOrg, strUI);` on the chat box's text, whose limit is 100 characters of UTF-32 and therefore up to 400 bytes of UTF-8, so a long Korean line overruns the block before any packet is built. Found by the adversarial review of the string-cap fix on 2026-09-06; the slice that bounds the chat text upstream owns it | open |
