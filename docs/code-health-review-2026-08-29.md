...
- **Half the shipped data is still packed, and this port cannot unpack it.** `CRarFile` no longer reads archives at all: `SetRAR()` keeps only the directory containing the `.rpk` and `Open()` does a plain `fopen(directory + filename)`, so it needs each archive's contents extracted flat beside it. `Data/Info` and `Data/ui/spk` are extracted; `Data/ui/txt` holds nothing but `Help.rpk`, `TutorialEtc.rpk`, `Item.rpk`, `Skill.rpk`, `Book.rpk`, `progress.rpk` and `title.rpk`, and `Data/ui/xml` is empty. Every lookup into those directories fails with `[RARFile ERROR]`, so quest data (`SimpleGQuest.xml`), chat help (`commoningame.txt`, `<race>ingame.txt`) and everything else inside them is simply absent at runtime.
  ...
  **Reader safety follow-up (2026-09-21):** archive extraction/listing remains open. The extracted-file reader now checks seeks, sizes and exact reads, rejecting negative or oversized requests without advancing its cursor or changing the destination. Files and temporary buffers have scoped ownership, failed opens and source changes close previous data, and shallow copying is disabled. `OpenText` decodes a file once under the declared resource policy with a 16 MiB input cap and clips lines at UTF-8 scalar boundaries; existing `Open` consumers continue reading raw bytes. Tests reproduced seven read-bound failures and four decoding/clipping failures. A separate ASan test reproduced the tutorial's deletion of the static list returned by `GetList`; the empty stub list is now caller-owned and may outlive its reader. Plain-text callers still need migration before the renderer's legacy fallback can be retired.
