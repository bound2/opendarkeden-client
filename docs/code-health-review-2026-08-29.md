...
| `GameMain.cpp` hands a string literal to the chat handler, which `delete[]`s it | `Client/GameMain.cpp` ~2870 does `SendMessage(UI_CHAT_RETURN, CLD_NORMAL, 0, (void*)"*command ghost on")`, and `Execute_UI_CHAT_RETURN` ends with `DeleteNewArray(void_ptr)` because the UI's own sender passes a `new char[]`. Reachable whenever `bCompetence && !bCompetenceShape` in a non-competence zone. Found by the adversarial review of the chat-bound fix on 2026-09-06 | open |
| `C_VS_UI_GAME_COMMON` reads the chat buffer after the handler has freed it | `VS_UI/src/VS_UI_GameCommon.cpp` ~4570 reads `pTempstr[0]` after `SendMessage(UI_CHAT_RETURN, ...)` returns, and `Execute_UI_CHAT_RETURN` has `DeleteNewArray`'d that buffer by then: a use after free on every chat line sent. Found while folding the chat-bound fix on 2026-09-06 | open |
